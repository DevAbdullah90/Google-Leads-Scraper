...
async def collect_business_urls(
    ...
    for attempt in range(1, MAX_SCROLL_ATTEMPTS + 1):
        ...
        # Log each scroll iteration
        if new_count > 0:
            no_new_streak = 0
            logger.debug(
                "Scroll %3d/%d | +%2d new | total=%d | selector=%r",
                attempt, MAX_SCROLL_ATTEMPTS, new_count, len(urls),
                link_selector_used or "none",
            )
        else:
            no_new_streak += 1
            logger.debug(
                "Scroll %3d/%d |  +0 new | total=%d | no-new streak=%d/%d",
                attempt, MAX_SCROLL_ATTEMPTS, len(urls), no_new_streak, max_no_new,
            )
    ...
    logger.info(
        "Collection complete: %d URLs in %.1fs | stop reason: %s",
